...
import { createProviderSchema } from "@/ai/contracts/provider-registry";

export async function GET(request: NextRequest) {
  try {
    await requireAdmin(request.headers);
    const providers = await modelRegistryService.listProviders();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const provider = await modelRegistryService.createProvider(
      parseResult.data,
      authContext.user.id,
    );
